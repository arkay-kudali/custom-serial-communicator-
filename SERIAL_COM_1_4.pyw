...
class SerialGUI:
    ...
    def update_ports(self):
        ...
        if self.ports_list:
            ...
            self.com_port_menu['values'] = []  # Clear previous values
            self.selected_port.set("")
            self.canvas.config(bg="red")  # Keep light red if no ports found
    ...
    def connect_serial(self):
        ...
        try:
            ...
            self.canvas.config(bg="green")  # Change light to green on success
            self.connect_button.config(text="Disconnect")  # Change button text
            self.is_connected = True  # Set state to connected
        except Exception:
            self.canvas.config(bg="red")  # Change light to red on failure
    
    def disconnect_serial(self):
        # Close the serial connection
        if self.serial_connection and self.serial_connection.is_open:
            self.serial_connection.close()
        self.canvas.config(bg="red")  # Change light to red when disconnected
        self.connect_button.config(text="Connect")  # Change button text
        self.is_connected = False  # Set state to disconnected
    ...
    def send_data(self):
        ...
        if self.serial_connection and self.serial_connection.is_open:
            ...
            self.canvas.config(bg="red")  # Ensure light is red if not connected
    
    def close_serial(self):
        # Ensure serial port is closed on exit
        self.disconnect_serial()
        self.root.destroy()  # Kill the program
